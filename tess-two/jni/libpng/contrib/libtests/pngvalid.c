...
#  ifdef PNG_USER_MEM_SUPPORTED
    if (!ps->speed)
        ps->pread = png_create_read_struct_2(PNG_LIBPNG_VER_STRING, ps,
                                             store_error, store_warning, &ps->read_memory_pool,
                                             store_malloc,
                                             store_free);
       ...
       */
                pp = set_store_for_read(d.ps, &pi, d.id,
                                        d.do_interlace ? (d.ps->progressive ?
                                                          ...
                                                          "pngvalid sequential deinterlacer")
                                                       : (d.ps->progressive ?
                                                          "progressive reader"
                                                                            : "sequential reader"));
    ...
    for (; bdlo <= bdhi; ++bdlo) {
        ...
        for (interlace_type = PNG_INTERLACE_NONE;
            ...
            standard_test(&pm->this, FILEID(colour_type, DEPTH(bdlo), 0/*palette*/,
                                            interlace_type, 0, 0, 0), do_read_interlace,
                          pm->use_update_info);
          ...
          */
            if (in_pixel.red != out_pixel.red)
                transform_range_check(pp, r, g, b, a, in_pixel.red, in_pixel.redf,
                                      ...
                                      dp->pm->limit + 1. / (2 * ((1U << in_pixel.red_sBIT) - 1)),
                                      "red/gray",
                                      digitization_error);
            ...
            if ((out_ct & PNG_COLOR_MASK_COLOR) != 0 &&
                ...
                transform_range_check(pp, r, g, b, a, in_pixel.green,
                                      in_pixel.greenf, out_pixel.green, sample_depth,
                                      in_pixel.greene,
                                      dp->pm->limit + 1. / (2 * ((1U << in_pixel.green_sBIT) - 1)),
                                      "green",
                                      digitization_error);
            ...
            if ((out_ct & PNG_COLOR_MASK_COLOR) != 0 &&
                ...
                transform_range_check(pp, r, g, b, a, in_pixel.blue, in_pixel.bluef,
                                      ...
                                      dp->pm->limit + 1. / (2 * ((1U << in_pixel.blue_sBIT) - 1)),
                                      "blue",
                                      digitization_error);
            ...
            if ((out_ct & PNG_COLOR_MASK_ALPHA) != 0 &&
                ...
                transform_range_check(pp, r, g, b, a, in_pixel.alpha,
                                      in_pixel.alphaf, out_pixel.alpha, sample_depth,
                                      in_pixel.alphae,
                                      dp->pm->limit + 1. / (2 * ((1U << in_pixel.alpha_sBIT) - 1)),
                                      "alpha",
                                      digitization_error);
        } /* pixel (x) loop */
    } /* row (y) loop */
...
image_transform_default_add(image_transform *this,
                            const image_transform **that, png_byte colour_type,
                            png_byte bit_depth) {
    UNUSED(colour_type)
    UNUSED(bit_depth)
...
gamma_test(png_modifier *pmIn, const png_byte colour_typeIn,
    ...
    Try {
       ...
       */
            ...
            } Catch(fault)modifier_reset(voidcast(png_modifier*, (void *) fault));
}
                    ...
                    for (j = 0; j < pm->ngamma_tests; ++j)
                        if (i != j) {
                            gamma_transform_test(pm, colour_type, bit_depth, npalette,
                                                 pm->interlace_type, 1 / pm->gammas[i],
                                                 pm->gammas[j],
