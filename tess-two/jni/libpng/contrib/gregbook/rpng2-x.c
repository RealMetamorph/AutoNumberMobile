...
int main(int argc, char **argv) {
    ...
    if (!(infile = fopen(filename, "rb"))) {
        ...
        if (incount < 8 || !readpng2_check_sig(inbuf, 8)) {
            ...
            ++error;
        } else { Trace((stderr, "about to call XOpenDisplay()\n"))
    ...
    for (;;) { Trace((stderr, "about to call readpng2_decode_data()\n"))
        if (readpng2_decode_data(&rpng2_info, inbuf, incount))
            ++error;Trace((stderr, "done with readpng2_decode_data()\n"))

        if (error || incount != INBUFSIZE || rpng2_info.state == kDone) {
            if (rpng2_info.state == kDone) { Trace((stderr, "done decoding PNG image\n"))
    ...
    fclose(infile);Trace((stderr, "about to call readpng2_cleanup()\n"))
    readpng2_cleanup(&rpng2_info);
    ...
    if (rpng2_info.state >= kWindowInit) { Trace(
                (stderr, "entering final wait-for-quit-event loop\n"))
        do {
...
static void rpng2_x_init(void) {
    ...
    Trace((stderr, "beginning rpng2_x_init()\n"))Trace(
            (stderr, "  rowbytes = %d\n", rpng2_info.rowbytes))Trace(
            (stderr, "  width  = %ld\n", rpng2_info.width))Trace(
            (stderr, "  height = %ld\n", rpng2_info.height))
...
static int rpng2_x_create_window(void) {
    ...
    if (depth != 16 && depth != 24 && depth != 32) {
        ...
        visual_list = XGetVisualInfo(display,
                                     VisualScreenMask | VisualDepthMask, &visual_info,
                                     &visuals_matched);
...
static void rpng2_x_redisplay_image(ulg startcol, ulg startrow,
    ...
    Trace((stderr, "beginning display loop (image_channels == %d)\n",
            rpng2_info.channels))Trace(
            (stderr, "   (width = %ld, rowbytes = %d, ximage_rowbytes = %d)\n",
                    rpng2_info.width, rpng2_info.rowbytes, ximage_rowbytes))Trace(
            (stderr, "   (bpp = %d)\n", ximage->bits_per_pixel))Trace(
            (stderr, "   (byte_order = %s)\n", ximage->byte_order == MSBFirst ?
                                               "MSBFirst" : (ximage->byte_order == LSBFirst
                                                             ? "LSBFirst" : "unknown")))
