...
wndclass.
hbrBackground = NULL; /* (HBRUSH) GetStockObject (GRAY_BRUSH); */
wndclass.
lpszMenuName = szProgName;
wndclass.
lpszClassName = szProgName;
...
if (szCmdLine[0] == '"')
strncpy (szCmdFileName, szCmdLine
+ 1,
strlen(szCmdLine)
- 2);
else
...
&pPngFileList, &iPngFileCount,
&iPngFileIndex);

/* load the image from file */

if (!
LoadImageFile (hwnd, szImgPathName,
...
DisplayImage (hwnd,
&pDib, &pDiData, cxWinSize, cyWinSize,
...
IDM_FILE_SAVE, MF_ENABLED);
else
EnableMenuItem (hMenu,
IDM_FILE_SAVE, MF_GRAYED);
...
if (!
PngFileOpenDlg (hwnd, szImgPathName, szTitleName
))
return 0;
...
if (!
PngFileSaveDlg (hwnd, szImgPathName, szTitleName
))
return 0;
...
if (
SearchPngList (pPngFileList, iPngFileCount,
&iPngFileIndex,
NULL, szImgPathName))
{
if (
strcmp (szImgPathName,
"") == 0)
return 0;
...
if (bStretched)
CheckMenuItem (hMenu,
IDM_OPTIONS_STRETCH, MF_CHECKED);
else
CheckMenuItem (hMenu,
IDM_OPTIONS_STRETCH, MF_UNCHECKED);
