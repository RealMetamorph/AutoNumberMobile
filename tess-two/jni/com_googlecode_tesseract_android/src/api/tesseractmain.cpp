...
void SetVariablesFromCLArgs(tesseract::TessBaseAPI * api, int
argc,
char **argv
) {
...
renderers->push_back(new
tesseract::TessOsdRenderer(outputbase)
);
} else {
bool b;
...
renderers->push_back(
new
tesseract::TessHOcrRenderer(outputbase, font_info
));
}
...
renderers->push_back(
new
tesseract::TessTsvRenderer(outputbase, font_info
));
}
...
renderers->push_back(
new
tesseract::TessPDFRenderer(outputbase, api
->
...
if (b) {
renderers->push_back(new
tesseract::TessUnlvRenderer(outputbase)
);
}
...
renderers->push_back(new
tesseract::TessTextRenderer(outputbase)
);
}
}
