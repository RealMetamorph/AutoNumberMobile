...
if (res_it->
IsAtBeginningOf(RIL_TEXTLINE)
) {
int x1, y1, x2, y2;
res_it->
Baseline(RIL_TEXTLINE, &x1, &y1, &x2, &y2
);
...
res_it->
Baseline(RIL_WORD, &word_x1, &word_y1, &word_x2, &word_y2
);
...
int font_id;
res_it->
WordFontAttributes(&bold, &italic, &underlined, &monospace,
...
char textfont[20];
snprintf(textfont,
sizeof(textfont), "/f-0-0 %d Tf ", fontsize);
pdf_str +=
textfont;
