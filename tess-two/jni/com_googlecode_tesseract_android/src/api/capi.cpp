...
* TESS_CALL TessTextRendererCreate(const char *outputbase) {
    return new TessTextRenderer(outputbase);
}

TESS_API TessResultRenderer
* TESS_CALL TessHOcrRendererCreate(const char *outputbase) {
    return new TessHOcrRenderer(outputbase);
}

TESS_API TessResultRenderer
* TESS_CALL TessHOcrRendererCreate2(const char *outputbase, BOOL font_info) {
    return new TessHOcrRenderer(outputbase, font_info);
}

TESS_API TessResultRenderer
* TESS_CALL TessPDFRendererCreate(const char *outputbase, const char *datadir) {
    return new TessPDFRenderer(outputbase, datadir);
}

TESS_API TessResultRenderer
* TESS_CALL TessUnlvRendererCreate(const char *outputbase) {
    return new TessUnlvRenderer(outputbase);
}

TESS_API TessResultRenderer
* TESS_CALL TessBoxTextRendererCreate(const char *outputbase) {
...
TessResultRendererInsert(TessResultRenderer * renderer, TessResultRenderer * next) {
    renderer->insert(next);
}

TESS_API TessResultRenderer
* TESS_CALL TessResultRendererNext(TessResultRenderer
* renderer)
{
return renderer->
...
TessResultRendererAddImage(TessResultRenderer * renderer, TessBaseAPI * api) {
...
TESS_API size_t
TESS_CALL TessBaseAPIGetOpenCLDevice(TessBaseAPI
* handle,
void **device
)
{
return handle->
getOpenCLDevice(device);
}
...
TESS_API Pix
* TESS_CALL TessBaseAPIGetInputImage(TessBaseAPI
* handle)
{
return handle->
...
TESS_API BOOL TESS_CALL
TessBaseAPIGetIntVariable(const TessBaseAPI *handle, const char *name, int *value) {
    return handle->GetIntVariable(name, value) ? TRUE : FALSE;
}

TESS_API BOOL TESS_CALL
TessBaseAPIGetBoolVariable(const TessBaseAPI *handle, const char *name, BOOL *value) {
    bool boolValue;
...
TESS_API BOOL TESS_CALL
TessBaseAPIGetDoubleVariable(const TessBaseAPI *handle, const char *name, double *value) {
...
TESS_API BOOL TESS_CALL
TessBaseAPIPrintVariablesToFile(const TessBaseAPI *handle, const char *filename) {
...
i++) {
varNames.
push_back(STRING(vars_vec[i])
);
varValues.
push_back(STRING(vars_values[i])
);
}
}

return handle->
Init(datapath, language, mode, configs, configs_size, &varNames, &varValues, set_only_non_debug_params
);
}
...
const char *datapath,
const char *language, TessOcrEngineMode
oem,
char **configs,
int configs_size
)
{
return handle->
Init(datapath, language, oem, configs, configs_size,
NULL, NULL, false);
}
...
TESS_API void TESS_CALL TessBaseAPISetPageSegMode(TessBaseAPI * handle, TessPageSegMode
mode)
{
handle->
SetPageSegMode(mode);
...
{
return handle->
TesseractRect(imagedata, bytes_per_pixel, bytes_per_line, left, top, width, height
);
}
...
TESS_API void TESS_CALL TessBaseAPISetSourceResolution(TessBaseAPI * handle, int
ppi)
{
handle->
...
TESS_API void TESS_CALL TessBaseAPISetRectangle(TessBaseAPI * handle, int
left,
int top,
int width,
int height
...
TESS_API void TESS_CALL
TessBaseAPISetThresholder(TessBaseAPI * handle, TessImageThresholder * thresholder) {
...
{
return handle->
GetTextlines(raw_image, raw_padding, pixa, blockids, paraids
);
}
...
{
return handle->
GetComponentImages(level, text_only
!= FALSE, pixa, blockids);
}

TESS_API struct Boxa *
        TESS_CALL TessBaseAPIGetComponentImages1(TessBaseAPI * handle,
...
* TESS_CALL TessBaseAPIAnalyseLayout(TessBaseAPI
* handle)
{
return handle->
...
int timeout_millisec, TessResultRenderer
* renderer)
{
if (handle->
ProcessPages(filename, retry_config, timeout_millisec, renderer
))
return TRUE;
else
return FALSE;
...
{
if (handle->
ProcessPage(pix, page_index, filename, retry_config, timeout_millisec, renderer
))
return TRUE;
else
return FALSE;
...
* TESS_CALL TessBaseAPIGetIterator(TessBaseAPI
* handle)
{
return handle->
...
* TESS_CALL TessBaseAPIGetMutableIterator(TessBaseAPI
* handle)
{
return handle->
...
TESS_API char *TESS_CALL TessBaseAPIGetHOCRText(TessBaseAPI * handle, int
page_number)
{
return handle->GetHOCRText(NULL, page_number);
}

TESS_API char *TESS_CALL TessBaseAPIGetBoxText(TessBaseAPI * handle, int
page_number)
{
return handle->
GetBoxText(page_number);
}
...
TESS_API BOOL TESS_CALL TessBaseAPIAdaptToWordStr(TessBaseAPI * handle, TessPageSegMode
mode,
const char *wordstr
)
{
return handle->
AdaptToWordStr(mode, wordstr
) ? TRUE : FALSE;
}
...
TESS_API BOOL TESS_CALL
TessBaseAPIGetTextDirection(TessBaseAPI * handle, int * out_offset, float * out_slope) {
    return handle->GetTextDirection(out_offset, out_slope) ? TRUE : FALSE;
}

TESS_API void TESS_CALL TessBaseAPISetDictFunc(TessBaseAPI * handle, TessDictFunc
f)
{
handle->
SetDictFunc(f);
...
TESS_API void TESS_CALL TessBaseAPISetProbabilityInContextFunc(TessBaseAPI * handle,
                                                               TessProbabilityInContextFunc
f)
{
handle->
SetProbabilityInContextFunc(f);
}
...
TESS_API void TESS_CALL
TessBaseAPIGetFeaturesForBlob(TessBaseAPI * handle, TBLOB * blob, INT_FEATURE_STRUCT * int_features,
                              int * num_features, int * FeatureOutlineIndex) {
    handle->GetFeaturesForBlob(blob, int_features, num_features, FeatureOutlineIndex);
}

TESS_API ROW
* TESS_CALL TessFindRowForBox(BLOCK_LIST
* blocks,
int left,
int top,
int right,
...
{
return
TessBaseAPI::FindRowForBox(blocks, left, top, right, bottom
);
}

TESS_API void TESS_CALL TessBaseAPIRunAdaptiveClassifier(TessBaseAPI * handle, TBLOB * blob, int
num_max_matches,
int *unichar_ids,
float *ratings,
...
{
handle->
RunAdaptiveClassifier(blob, num_max_matches, unichar_ids, ratings, num_matches_returned
);
}

TESS_API const char *TESS_CALL TessBaseAPIGetUnichar(TessBaseAPI * handle, int
unichar_id)
{
return handle->
GetUnichar(unichar_id);
}
...
TESS_API ROW
* TESS_CALL TessMakeTessOCRRow(float baseline, float xheight, float descender, float ascender) {
    return TessBaseAPI::MakeTessOCRRow(baseline, xheight, descender, ascender);
}

TESS_API TBLOB
* TESS_CALL TessMakeTBLOB(struct Pix *pix) {
    return TessBaseAPI::MakeTBLOB(pix);
}

TESS_API void TESS_CALL TessNormalizeTBLOB(TBLOB * tblob, ROW * row, BOOL
numeric_mode)
{
...
TESS_API void TESS_CALL TessBaseAPISetMinOrientationMargin(TessBaseAPI * handle, double
margin)
{
handle->
...
TESS_API void TESS_CALL
TessBaseGetBlockTextOrientations(TessBaseAPI * handle, int * *block_orientation,
...
TESS_API BOOL TESS_CALL TessPageIteratorNext(TessPageIterator * handle, TessPageIteratorLevel
level)
{
return handle->
Next(level)
...
TESS_API BOOL TESS_CALL
TessPageIteratorIsAtBeginningOf(const TessPageIterator *handle, TessPageIteratorLevel level) {
    return handle->IsAtBeginningOf(level) ? TRUE : FALSE;
}

TESS_API BOOL TESS_CALL
TessPageIteratorIsAtFinalElement(const TessPageIterator *handle, TessPageIteratorLevel level,
                                 TessPageIteratorLevel element) {
    return handle->IsAtFinalElement(level, element) ? TRUE : FALSE;
}

TESS_API BOOL TESS_CALL
TessPageIteratorBoundingBox(const TessPageIterator *handle, TessPageIteratorLevel level,
...
TESS_API struct Pix *
TESS_CALL TessPageIteratorGetImage(const TessPageIterator *handle, TessPageIteratorLevel level,
                                   int padding,
...
TESS_API BOOL TESS_CALL
TessPageIteratorBaseline(const TessPageIterator *handle, TessPageIteratorLevel level,
                         int *x1, int *y1, int *x2, int *y2) {
    return handle->Baseline(level, x1, y1, x2, y2) ? TRUE : FALSE;
}

TESS_API void TESS_CALL
TessPageIteratorOrientation(TessPageIterator * handle, TessOrientation * orientation,
...
TESS_API TessPageIterator
* TESS_CALL TessResultIteratorGetPageIterator(TessResultIterator
* handle)
{
return
handle;
...
TESS_API BOOL  TESS_CALL TessResultIteratorNext(TessResultIterator * handle, TessPageIteratorLevel
level)
{
return handle->
Next(level);
...
TessResultIteratorConfidence(const TessResultIterator *handle, TessPageIteratorLevel level) {
...
TESS_API const char *
TESS_CALL TessResultIteratorWordFontAttributes(const TessResultIterator *handle, BOOL *is_bold,
                                               BOOL *is_italic,
    ...
    const char *ret = handle->WordFontAttributes(&bool_is_bold, &bool_is_italic,
                                                 &bool_is_underlined, &bool_is_monospace,
                                                 &bool_is_serif,
