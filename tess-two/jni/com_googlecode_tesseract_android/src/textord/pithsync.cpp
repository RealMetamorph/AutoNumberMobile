...
double check_pitch_sync2(                          //find segmentation
    ...
    for (offset = 0; offset <= pitch_error; offset++, x++)
        //not quite free
        cutpts[x - array_origin].setup(cutpts, array_origin, projection, zero_count, pitch, x,
                                       offset);
...
double check_pitch_sync3(                          //find segmentation
        inT16
projection_left,    //edges //to be considered 0
inT16 projection_right,
        inT16
zero_count,
inT16 pitch,              //pitch estimate
...
float projection_scale,   //scale factor
        inT16
&occupation_count,  //no of occupied cells
FPSEGPT_LIST *seg_list,   //output list
        inT16
...
for (
left_edge = projection_left;
projection->
pile_count (left_edge)
== 0
&&
left_edge<projection_right;
left_edge++);
for (
right_edge = projection_right;
projection->
pile_count (right_edge)
== 0
&& right_edge >
left_edge;
right_edge--);
...
setup (cutpts, array_origin, projection, zero_count, pitch, x,
0);
...
//not quite free
cutpts[x - array_origin].
setup (cutpts, array_origin, projection, zero_count, pitch, x, offset
);

best_cost = MAX_FLOAT32;
best_end = NULL;
for (
offset = -pitch_error, minindex = 0;
offset<pitch_error;
offset++, minindex++)
...
else if (cutpts[x - array_origin].fake_count ==
best_fake
        &&x
== best_right_x + 1
&& cutpts[x - array_origin].
...
do {
for (
x = best_end->position() - pitch + pitch_error;
x<best_end->
