...
EXTERN BOOL_VAR (textord_all_prop, FALSE,
...
EXTERN BOOL_VAR (textord_show_row_cuts, FALSE,
"Draw row-level cuts");
EXTERN BOOL_VAR (textord_show_page_cuts, FALSE,
...
EXTERN double_VAR (textord_projection_scale,
0.200, "Ding rate for mid-cuts");
EXTERN double_VAR (textord_balance_factor,
1.0,
...
float tune_row_pitch2(                             //find fp cells
    ...
    for (start = best_pixel - 2; start > best_pixel - best_pitch
                                 && sum_proj[textord_pitch_range +
                                             best_delta].pile_count(start % best_pitch) <=
                                    best_count;
         start--);
...
float compute_pitch_sd(                            //find fp cells
    ...
    do {
        ...
        if (pitsync_linear_version & 3)
            word_sync =
                    check_pitch_sync2(&start_it, blob_count, (inT16)
        initial_pitch, 2,
        ...
        word_sync =
                check_pitch_sync(&start_it, blob_count, (inT16)
        initial_pitch, 2,
                projection, &seg_list);
        if (testing_on) {
        ...
        for (seg_it.mark_cycle_pt(); !seg_it.cycled_list(); seg_it.forward()) {
            ...
            if (cell_it.empty() || segpos > cellpos + initial_pitch / 2) {
                ...
                while (!cell_it.empty() && segpos > cellpos + initial_pitch * 3 / 2) {
                    cell = new ICOORDELT(cellpos + (inT16)
                    initial_pitch, 0);
...
float compute_pitch_sd2(                            //find fp cells
    ...
    plot_it = blob_it;
    word_sync = check_pitch_sync2(&blob_it, blob_count, (inT16)
    initial_pitch,
...
void print_pitch_sd(                        //find fp cells
    ...
    blob_it = row_start;
    word_sync =
            check_pitch_sync2(&blob_it, total_blob_count, (inT16)
    initial_pitch, 2,
            projection, projection_left, projection_right,
...
void plot_fp_word(                  //draw block of words
    ...
    for (row_it.mark_cycle_pt(); !row_it.cycled_list(); row_it.forward()) {
        ...
        plot_word_decisions(to_win, (inT16)
        pitch, row);
    }
}
