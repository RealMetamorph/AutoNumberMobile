...
// Constant add-on for minimum gutter for ragged tabs.
    ...
    double_VAR(textord_tabvector_vertical_gap_fraction,
    0.5,
    "max fraction of mean blob width allowed for vertical gaps in vertical text");

    double_VAR(textord_tabvector_vertical_box_ratio,
    0.5,
...
// Create a constraint for the top or bottom of this TabVector.
    void TabConstraint::CreateConstraint(TabVector * vector, bool
    is_top) {
    ...
    it.
    add_to_end(constraint);
    if (is_top)
    vector->
    set_top_constraints(constraints);
    else
    vector->
    set_bottom_constraints(constraints);
}
