...
EXTERN INT_VAR (textord_fp_chop_error,
2,
"Max allowed bending of chop cells");
EXTERN double_VAR (textord_fp_chop_snap,
0.5,
...
void fixed_split_coutline(                        //chop the outline
        ...
        inT16
        chop_coord,       //place to chop
        float pitch_error,      //allowed deviation
        C_OUTLINE_IT
        *left_it,  //left half of chop
...
// so put the whole thing on the right.
        right_it->
                add_before_stay_put(srcline);
    } else {
// Needs real chopping.
        if (
                fixed_chop_coutline(srcline, chop_coord, pitch_error,
...
// impossible outlines from being created.
                    if (
                            fixed_chop_coutline(child, chop_coord,
                                                0.0f,
                                                &left_frags, &right_frags)) {
                        delete
                                child;
...
// Chop failed. Just use middle coord.
            if (srcbox.
                ...
                <= chop_coord * 2)
                left_it->
                        add_after_then_move(srcline);  // Stick whole in left.
            else
                right_it->
                        add_before_stay_put(srcline);
        }
        ...
        do {
            ...
            if (tail_index == length)
                ...
                x()
                 ...
                 !=
                 chop_coord && tail_index
                               != startindex);
        if (tail_index == startindex) {
            if (first_frag)
                return
                        FALSE;            //doesn't cross line
            else
                break;
        }
//#ifdef __UNIX__
        ASSERT_HOST(head_index
                    != tail_index);
//#endif
        if (!first_frag) {
...
void save_chop_cfragment(                            //chop the outline
        inT16
head_index,           //head of fragment
ICOORD head_pos,            //head of fragment
        inT16
tail_index,           //tail of fragment
ICOORD tail_pos,            //tail of fragment
        C_OUTLINE
*srcline,         //source of edgesteps
        ...
        if (top_frag->other_end != bottom_frag) {
            ...
            if (outline != NULL) {
                ...
                if (outline->
                    ...
                    > pitch_error)
                    dest_it->
                            add_after_then_move(outline);
                else
                    delete
                            outline;          // Make it disappear.
            }
        }
    }
    while (!child_it.
