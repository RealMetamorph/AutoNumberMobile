...
// so that the assert there never fails.
    ...
    BOOL_VAR(textord_tabfind_show_initialtabs,
    false, "Show tab candidates");
    BOOL_VAR(textord_tabfind_show_finaltabs,
    false, "Show tab vectors");
...
// and if so add it to the left/right tab boxes.
    ...
    bool TabFind::TestBoxForTabs(BLOBNBOX * bbox, int
    min_gutter_width,
    double tabfind_aligned_gap_fraction
    ) {
...
+ right_x)/2, (top_y + bottom_y)/2, radius);
...
while ((
neighbour = radsearch.NextRadSearch()
) != NULL) {
if (neighbour == bbox)
continue;
...
n_left<left_column_edge ||
       left_x < neighbour->left_rule() || right_x> neighbour
->
...
if (n_left<left_x && nbox.
y_overlap(box)
&& n_right >= target_right) {
maybe_ragged_left = false;
if (debug)
...
MAX_INT32 &&maybe_left_tab_up
== -
MAX_INT32 &&
        maybe_right_tab_down
== -
MAX_INT32 &&maybe_right_tab_up
== -MAX_INT32)
break;
}
if (is_left_tab || maybe_left_tab_up > 1 || maybe_left_tab_down > 1) {
bbox->
set_left_tab_type(TT_MAYBE_ALIGNED);
} else if (
maybe_ragged_left &&ConfirmRaggedLeft(bbox, min_ragged_gutter)
) {
bbox->
set_left_tab_type(TT_MAYBE_RAGGED);
} else {
bbox->
set_left_tab_type(TT_NONE);
}
if (is_right_tab || maybe_right_tab_up > 1 || maybe_right_tab_down > 1) {
bbox->
set_right_tab_type(TT_MAYBE_ALIGNED);
} else if (
maybe_ragged_right &&
        ConfirmRaggedRight(bbox, min_ragged_gutter)
) {
bbox->
set_right_tab_type(TT_MAYBE_RAGGED);
} else {
bbox->
set_right_tab_type(TT_NONE);
}
if (debug) {
...
== TT_MAYBE_ALIGNED ? "Aligned" :
(bbox->

left_tab_type()

== TT_MAYBE_RAGGED ? "Ragged" : "None"),
bbox->
...
// the left of bbox.
bool TabFind::ConfirmRaggedLeft(BLOBNBOX * bbox, int
min_gutter) {
TBOX search_box(bbox->bounding_box());
search_box.
set_right(search_box
.
...
- min_gutter);
return
NothingYOverlapsInBox(search_box, bbox
->
...
// Returns true if there is nothing in the rectangle of width min_gutter to
// the right of bbox.
bool TabFind::ConfirmRaggedRight(BLOBNBOX * bbox, int
min_gutter) {
TBOX search_box(bbox->bounding_box());
search_box.
set_left(search_box
