...
int STATS::top_n_modes(int max_modes,
                       GenericVector<KDPairInc < float, int>
...
++offset) {
if (!
GatherPeak(max_index
+ offset, buckets_, used.buckets_,
...
if (total_count > least_count || modes->
...
modes->
insert(KDPairInc<float, int>(peak_mean, total_count),
        target_index
);
