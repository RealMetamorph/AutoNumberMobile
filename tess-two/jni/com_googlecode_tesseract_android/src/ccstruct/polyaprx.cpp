...
EXTERN BOOL_VAR(poly_debug, FALSE,
"Debug old poly");
...
pos +=
prev_vec;
edgepts[epindex].flags[RUNLENGTH] =
count;
edgepts[epindex].
...
epdir >>= 4;
epdir &= 7;
edgepts[epindex].flags[DIR] =
epdir;
edgepts[epindex].
src_outline = c_outline;
edgepts[epindex].
start_step = prev_stepindex;
edgepts[epindex].
...
&& (
dir1 =
(edgept->prev->flags[DIR] - edgept->next->flags[DIR]) & 7
) != 2
&& dir1 != 6)
...
stopped = 1;             /*finished */
if (sum2 + sum1 > 2
&& linestart->prev->flags[DIR] ==
dir2
        &&(linestart
->prev->flags[RUNLENGTH] >
...
d23 = LENGTH (d23vec);
if (d01 > d23) {
edgefix2->flags[FLAGS] &=
~
FIXED;
fixed_count--;
}
else {
edgefix1->flags[FLAGS] &=
~
FIXED;
fixed_count--;
edgefix1 = edgefix2;
...
prev = linestart;
linestart->vec.
x = edgept->pos.x - linestart->pos.x;
linestart->vec.
y = edgept->pos.y - linestart->pos.y;
}
while (edgept != loopstart);
}
else
...
/*vector sum */
vecsum.
x = last->pos.x - edge->pos.x;
vecsum.
y = last->pos.y - edge->pos.y;
...
vec.
x = edge->vec.x;           /*accumulated vector */
vec.
y = edge->vec.y;
maxperp = 0;                   /*none yet */
squaresum = ptcount = 0;
