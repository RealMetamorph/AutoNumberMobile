...
STATISTICS *ComputeStatistics(inT16
N,
PARAM_DESC ParamDesc[], CLUSTER
* Cluster);
...
PROTOTYPE *NewEllipticalProto(inT16
N,
CLUSTER *Cluster,
        STATISTICS
*Statistics);

PROTOTYPE *NewMixedProto(inT16
N,
CLUSTER *Cluster, STATISTICS
*Statistics);

PROTOTYPE *NewSimpleProto(inT16
N,
CLUSTER *Cluster
);
...
FLOAT64 NormalDensity(inT32
x);

FLOAT64 UniformDensity(inT32
x);

FLOAT64 Integral(FLOAT64
f1,
FLOAT64 f2, FLOAT64
Dx);
...
i<SampleSize;
i++) {
Clusterer->ParamDesc[i].
Circular = ParamDesc[i].Circular;
Clusterer->ParamDesc[i].
NonEssential = ParamDesc[i].NonEssential;
Clusterer->ParamDesc[i].
Min = ParamDesc[i].Min;
Clusterer->ParamDesc[i].
Max = ParamDesc[i].Max;
Clusterer->ParamDesc[i].
Range = ParamDesc[i].Max - ParamDesc[i].Min;
Clusterer->ParamDesc[i].
HalfRange = Clusterer->ParamDesc[i].Range / 2;
Clusterer->ParamDesc[i].
MidRange =
...
Clusterer->
KDTree = MakeKDTree(SampleSize, ParamDesc);
...
KDStore (Clusterer
->KDTree, Sample->Mean, (char *) Sample);
...
FLOAT32 StandardDeviation(PROTOTYPE *Proto, uinT16 Dimension) {
    switch (Proto->Style) {
        case spherical:
            return ((FLOAT32)
            sqrt((double) Proto->Variance.Spherical));
        case elliptical:
            return ((FLOAT32)
...
FillBuckets (NormalBuckets, Cluster, i, &(Clusterer
->ParamDesc[i]),
Proto->Mean[i],
sqrt ((FLOAT64)
Proto->Variance.Elliptical[i]));
if (
DistributionOK (NormalBuckets)
)
continue;
...
MakeDimRandom (i, Proto, &(Clusterer
->ParamDesc[i]));
FillBuckets (RandomBuckets, Cluster, i, &(Clusterer
->ParamDesc[i]),
Proto->Mean[i], Proto->Variance.Elliptical[i]);
if (
DistributionOK (RandomBuckets)
)
continue;
...
FillBuckets (UniformBuckets, Cluster, i, &(Clusterer
->ParamDesc[i]),
Proto->Mean[i], Proto->Variance.Elliptical[i]);
if (
DistributionOK (UniformBuckets)
)
continue;
break;
}
...
Statistics->
AvgVariance = (float) pow((double) Statistics->AvgVariance,
                          1.0 / N);
...
1.0 / sqrt ((double) (2.0 *
PI *Proto
->Variance.Elliptical[i]));
Proto->Weight.Elliptical[i] = 1.0 / Proto->Variance.Elliptical[i];
Proto->TotalMagnitude *= Proto->Magnitude.Elliptical[i];
}
Proto->
LogMagnitude = log((double) Proto->TotalMagnitude);
Proto->
Style = elliptical;
return (Proto);
}                                // NewEllipticalProto
...
Proto = NewEllipticalProto(N, Cluster, Statistics);
Proto->
Distrib = (DISTRIBUTION *) Emalloc(N * sizeof(DISTRIBUTION));
...
Style = spherical;
Proto->
NumSamples = Cluster->SampleCount;
Proto->
Cluster = Cluster;
Proto->Cluster->
Prototype = TRUE;
...
return
kNormalMagnitude *exp(-0.5 * Distance * Distance / kNormalVariance);
}                                // NormalDensity
...
FLOAT64 UniformDensity(inT32
x) {
static FLOAT64 UniformDistributionDensity = (FLOAT64)
1.0 / BUCKETTABLESIZE;

if ((x >= 0.0) && (x <= BUCKETTABLESIZE))
return
UniformDistributionDensity;
else
return (FLOAT64) 0.0;
}                                // UniformDensity
