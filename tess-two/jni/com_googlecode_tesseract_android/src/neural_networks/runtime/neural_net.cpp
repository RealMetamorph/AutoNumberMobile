...
// Templatized for float and double Types
    ...
    bool NeuralNet::FastFeedForward(const Type *inputs,
                                    Type *outputs) {
        int node_idx = 0;
...
// Templatized for float and double Types
    template<typename Type>
    bool NeuralNet::FeedForward(const Type *inputs,
                                Type *outputs) {
            ...
            if (neurons_[node_idx].node_type() == Neuron::Input) {
                ...
                if (fabs(inputs_max_[node_idx] - inputs_min_[node_idx]) <
                    ...
                    node->bias = inputs_min_[node_idx] + (inputs_mean_[node_idx] *
                                                          (inputs_max_[node_idx] -
                                                           inputs_min_[node_idx]));
                }
            } else {
...
// This is the fast-read-only version of this function
    template<typename Type>
    bool NeuralNet::FastGetNetOutput(const Type *inputs,
                                     int output_id,
                                     Type *output) {
        // feed inputs in and offset them by the pre-computed bias
        int node_idx = 0;
...
// Templatized for float and double Types
    template<typename Type>
    bool NeuralNet::GetNetOutput(const Type *inputs,
                                 int output_id,
                                 Type *output) {
        // validate output id
